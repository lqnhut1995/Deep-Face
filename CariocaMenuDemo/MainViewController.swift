...
extension MainViewController: CariocaDelegate {
	func cariocamenu(_ menu: CariocaMenu, didSelect item: CariocaMenuItem, at index: Int) {
		...
		switch index {
		case 1:
			displayDemo(DemoScanViewController.fromStoryboard())
        case 2:
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let mainVC = storyboard.instantiateViewController(withIdentifier: "loginvc") as! LoginViewController
            UIView.transition(from: AppDelegate.sharedInstance().window!.rootViewController!.view, to: mainVC.view, duration: 1.0, options: [.transitionFlipFromLeft], completion: {
                _ in
